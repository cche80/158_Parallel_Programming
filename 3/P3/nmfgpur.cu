...
extern "C" {
	SEXP nmfgpuC(SEXP in_a, SEXP in_r, SEXP in_c, SEXP in_k, SEXP in_niters);
}
...
SEXP nmfgpuC(SEXP in_a, SEXP in_r, SEXP in_c, SEXP in_k, SEXP in_niters) {
	// setup cuda
	int r = INTEGER(in_r)[0];
	int c = INTEGER(in_c)[0];
	int k = INTEGER(in_k)[0];
	int niters = INTEGER(in_niters)[0];
	float* a = new float[r*c];
	for (int i = 0; i < r*c; i++) {
		a[i] = REAL(in_a)[i];	
	}

	SEXP w_h = PROTECT(allocVector(REALSXP, (r*k + k*c)));
